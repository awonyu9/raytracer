...
rsilver.diffuse *= .5
rsilver.ambient *= .5
scene.add(Sphere(pos=(2.5, -2, -20), radius=1, color=rsilver))

rbrass = Material(*BRASS)
rbrass.reflect = (.2, .2, .2)
scene.add(Sphere(pos=(0, 0, -25), radius=3, color=rbrass))

bmat = Material((.4, .4, .4), reflect=(.4, .4, .4))
scene.add(Box(pos=(0, -3.5, -20), size=(18, 1, 30), color=bmat))
